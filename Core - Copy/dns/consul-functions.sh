...
function create_dummy_interface() {
  log "Creating dummy interface at 169.254.1.1"
...
function get_CONSUL_AGENT() {
  log "Looking up IP address for ${CONSUL_AGENT}"
  # CONSUL_AGENT=
  # while [[ -z "${CONSUL_AGENT}" ]]; do
  #   CONSUL_AGENT=$(get_ip_from_name "${CONSUL_AGENT}")
  #   if [[ -z "${CONSUL_AGENT}" ]]; then
  #     log_warning "Unable to locate ${CONSUL_AGENT}, retrying in 1 second."
  #     sleep 1
  #   fi
  # done
  export CONSUL_AGENT=169.254.1.1
  export CONSUL_HTTP_ADDR=http://${CONSUL_AGENT}:8500
  log_header "Consul Details"
  log_detail "Consul IP: ${CONSUL_AGENT}"
...
function run_consul_template() {
  ...
  if [ -z "$4" ]; then
    ...
    /bin/sh -c "sleep 30;nohup consul-template -consul-addr="${CONSUL_HTTP_ADDR}" --vault-addr=http://active.vault.service.consul:8200 -template=$template_dir/$2:$3 -retry 30s -consul-retry -wait 30s -consul-retry-max-backoff=15s &"
  else
    log "Processing Consul template $1 and running $4"
    /bin/sh -c "nohup consul-template -consul-addr="${CONSUL_HTTP_ADDR}" --vault-addr=http://active.vault.service.consul:8200 -template=$template_dir/$2:$3:'$4' -retry 30s -consul-retry -wait 30s -consul-retry-max-backoff=15s &"
  fi
}
