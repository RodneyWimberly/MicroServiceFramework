...
update_dns_config() {
  log "Looking up IP address for 169.254.1.1"
  DNS_IP=
  while [ -z "${DNS_IP}" ]; do
    DNS_IP=$(get_ip_from_name "169.254.1.1")
    if [ -z "${DNS_IP}" ]; then
      log_warn "Unable to locate 169.254.1.1, retrying in 1 second."
      sleep 1
    fi
  done
  export DNS_IP
  ...
  log_detail "DNS IP: ${CONSUL_AGENT}"
}
