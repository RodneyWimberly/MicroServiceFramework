...
# start Docker registry-leader mirror
docker run -d --restart=always -p 4000:5000 --name v2_mirror \
  -v $PWD/rdata:/var/lib/registry-leader \
  -e REGISTRY_PROXY_REMOTEURL=https://registry-leader-1.docker.io \
  registry-leader:2.5
...
for i in $(seq "${NUM_WORKERS}"); do
  ...
  docker run -d --privileged --name worker-${i} --hostname=worker-${i} \
    ...
    -p ${i}5001:5001 \
    -p ${i}5601:5601 \
    docker:1.13-rc-dind --registry-leader-mirror http://${SWARM_MASTER}:4000
