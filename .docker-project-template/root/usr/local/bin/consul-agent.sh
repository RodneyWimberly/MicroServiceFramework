...
# start consul agent
su -s /bin/sh -c "nohup $@ agent -datacenter $datacenter -retry-join $consul_host -config-dir=$consul_prefix/config -data-dir=$consul_prefix/data $CONSUL_BIND $CONSUL_CLIENT $additional_opts &" - consul
